...
First, we simulate data for this vignette. Specifically, we simulate a dataset of 1000 cells, about half of which are from the diseased-condition. Each cell is defined by a feature vector of 10 features. The disease-associated cells are have half of their features shifted from those of non-disease-associated cells by adding a constant value of 4 to each. The data are simulated such that the disease-associated cells are only present in "patients" who have the disease, but non-disease associated cells are present in both healthy and sick "patients".
...
This package provides several functions for calculating quantities that are needed to apply the Mixture LASSO/EMMIL/AlgorithmName to your own model, as long as that model's objective is to optimize the binary classification log-likelihood.  Here are some of those functions: 
...
* `emmil_initialize_y`
* `emmil_calculate_sample_label_adjustment`
* `emmil_plot_lls`

# The high-level API 

This package also provides a high-level API that can be used to fit `glmnet`, `glm`, or `nnet` models using the Mixture LASSO/EMMIL/AlgorithmName schema. 
...
lls <- emmil_get_log_likelihoods(glmnet_model)
```
...
glmnet_predictions <- 
  ...
  emmil_adjust_predictions_case_control(
    ...
    rho = rho, 
    zeta = zeta, 
    # let EMMIL know that the predictions are probabilities, not logits (
